...
WEBLATE_PROMOTION_WORKFLOW_TEMPLATE = (
    REPO_ROOT / "examples" / "github-actions" / "weblate_translation_promote.yml"
)
...
def write_version_branch_workflow(
    *,
    ...
    workflow_path = checkout / ".github" / "workflows" / "document_template_translation_sync.yml"
    workflow_path.parent.mkdir(parents=True, exist_ok=True)
    workflow_path.write_text(workflow, encoding="utf-8")


def write_weblate_promotion_workflow(
    *,
    checkout: Path,
    config: TranslationRepositoryConfig,
    version: str,
) -> None:
    """Render the Weblate review-branch promotion workflow."""

    paths = version_paths(config, version)
    target_branch = version_branch(config, version)
    weblate_branch = f"weblate/{version}"
    ...
    replacements = {
        ...
        "TOOLING_REF: main": f"TOOLING_REF: {_yaml_scalar(config.tooling.ref)}",
        "TARGET_BRANCH: translation/v1.30.0": (f"TARGET_BRANCH: {_yaml_scalar(target_branch)}"),
        "WEBLATE_BRANCH: weblate/v1.30.0": (f"WEBLATE_BRANCH: {_yaml_scalar(weblate_branch)}"),
        "COMPACT_TEMPLATE_DIR: workspace/document-templates/compact/dsw-science-europe-1.30.0": (
            f"COMPACT_TEMPLATE_DIR: {_yaml_scalar(paths.compact_template_dir.as_posix())}"
        ),
        "EXPANDED_TEMPLATE_DIR: workspace/document-templates/expanded/dsw-science-europe-1.30.0": (
    ...
    for old, new in replacements.items():
        if old not in workflow:
            raise SystemExit(f"Weblate promotion workflow template is missing expected text: {old}")
        workflow = workflow.replace(old, new)

    workflow_path = checkout / ".github" / "workflows" / "weblate_translation_promote.yml"
    workflow_path.parent.mkdir(parents=True, exist_ok=True)
    workflow_path.write_text(workflow, encoding="utf-8")
